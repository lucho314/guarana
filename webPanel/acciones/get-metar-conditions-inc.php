...
############### Tomo el metar desde el simulador. Si está vacío, tomo el del aeropuerto  


        include '../lib/variables.php';

        $url = $nro . "/json/environment/metar/data";
        $json = file_get_contents($url);

        $datos = json_decode($json);
        //print_r($datos);
        $metar_limpio = $datos->value;
        // echo  $url = $ip."/environment/metar/";
        /* $page = file_get_contents($url);
          ...
          $cadena_de_texto = $data;
          $cadena_buscada = 'data">(';
          $posicion_coincidencia = strpos($cadena_de_texto, $cadena_buscada);

          //47

          $metar1 = substr($cadena_de_texto, 47);

          $metar_limpio = substr($metar1, 0, -11);

         */

        if (!empty($metar_limpio)) {

            $raw_metar = $metar_limpio;
        } else
        ...
        $age = abs(time() - $metarDate); // age in seconds
...
// thanks to Tom at Carterlake.org for this script fragment
    ...
    if (!$useFopen) {
        ...
        if (isset($urlParts['query'])) {
            $resourcePath .= "?" . $urlParts['query'];
        }
        if (isset($urlParts['fragment'])) {
            $resourcePath .= "#" . $urlParts['fragment'];
        }
